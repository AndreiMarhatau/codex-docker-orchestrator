...
describe('orchestrator resume', () => {
  ...
  it('overrides docker socket usage on resume', async () => {
    ...
    const meta = {
      ...
      error: 'Previous startup failure.',
      runs: [
        {
          runId: 'run-001',
          prompt: 'start',
  ...
  it('preserves previous failure state when resume setup fails before deferred start', async () => {
    const orchHome = await createTempDir();
    const exec = createMockExec({ branches: ['main'] });
    const spawn = createMockSpawn();
    const orchestrator = new Orchestrator({ orchHome, exec, spawn });
    ...
    await expect(orchestrator.resumeTask('task-1', 'Continue')).rejects.toThrow(/auth unavailable/);

    const updated = JSON.parse(await fs.readFile(path.join(taskDir, 'meta.json'), 'utf8'));
    expect(updated.status).toBe('failed');
    expect(updated.error).toBe('Previous startup failure.');
