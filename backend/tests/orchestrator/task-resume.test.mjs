...
describe('orchestrator resume', () => {
  ...
  it('does not mark the task running when resume setup fails before spawn', async () => {
    const orchHome = await createTempDir();
    const exec = createMockExec({ branches: ['main'] });
    const spawn = createMockSpawn();
    ...
    await expect(orchestrator.resumeTask('task-1', 'Continue')).rejects.toThrow(
      /instructions file not found/i
    );
