...
describe('orchestrator resume sidecar cleanup', () => {
  it('cleans up first-time sidecar when resume setup fails', async () => {
    const orchHome = await createTempDir();
    const calls = [];
  ...
  it('stops existing sidecar when resume setup fails after start', async () => {
    const orchHome = await createTempDir();
    const calls = [];
    const exec = async (command, args) => {
      calls.push({ command, args });
    ...
    await expect(orchestrator.resumeTask('task-1', 'Continue')).rejects.toThrow(/env failed/);
