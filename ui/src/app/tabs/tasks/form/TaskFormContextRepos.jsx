      ...
      {contextRepos.map((entry, index) => {
        ...
        return (
          <Stack key={`context-repo-${index}`} spacing={1.1} className="task-compose-nested-panel">
            <Stack direction="row" spacing={1} alignItems="flex-start" className="task-compose-repo-row">
              <TextField
                select
                size="small"
                label="Environment"
                ...
                disabled={loading}
                className="task-compose-field task-compose-repo-field"
              >
                {envs.map((env) => (
                  <MenuItem
                    key={env.envId}
              ...
              <IconButton
                ...
                disabled={loading}
                className="task-compose-repo-remove"
              >
