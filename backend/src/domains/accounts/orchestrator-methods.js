const { USAGE_TRIGGER_PROMPT, runAccountUsageTrigger } = require('./usage-trigger');
const { readAccountRateLimits } = require('./rate-limits-client');
...
function attachAccountMethods(Orchestrator) {
  ...
  Orchestrator.prototype.getAccountRateLimits = async function getAccountRateLimits() {
    ...
    if (!activeAccount?.id) {
      const { noActiveAccountError } = require('../../orchestrator/errors');
