...
- The `reviewer` agent is responsible for reviewing the uncommitted changes. The orchestrator should use that review to decide whether more developer work is required.
...
- If a task is worth showing visually to the user, the orchestrator may instruct the `developer` agent to run whatever is needed to produce screenshots or other demonstration artifacts, and those files must be saved in `~/.artifacts`.
