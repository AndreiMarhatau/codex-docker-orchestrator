...
### systemd (systemctl)
Copy `codex-docker-orchestrator.service` to `/etc/systemd/system/` and update paths/user for your machine, then:
```
...
- Ensures Homebrew Node is on `PATH` (fixes occasional node resolution quirks).
- Installs dependencies with `npm ci`, builds the UI, then starts the backend with `node src/server.js`.
- Restarts automatically on failure.

`ORCH_PORT` defaults to 8080 in the backend config, so you can drop the explicit `Environment=ORCH_PORT=8080` line if you prefer.
